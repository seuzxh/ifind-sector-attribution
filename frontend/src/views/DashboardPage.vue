...
async function loadDashboard(mySeq: number) {
  ...
  try {
    if (mode.value === 'history') {
      ...
      data = await getHistoryDashboard(date, 10, false, isCustom.value ? 'custom' : 'sector')
    } else {
      // 实时模式：走分时接口
      ...
      data = isCustom.value
        ...
        : await getRealtimeDashboard(params)
    }
  } catch (e: any) {
...
watch(() => route.name, (name, oldName) => {
  if (name === oldName || !['sector', 'custom'].includes(String(name))) return
  activeCode.value = ''
  membersTab.value = 'top'
