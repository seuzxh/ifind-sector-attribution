...
### 9.4 看板板块分类去重（2026-08-18 增补）

**一句话**：板块强度监控的强/弱榜用 KG 做三层展示规则，解决"半导体材料/设备/分立器件/MicroLED 四兄弟霸榜"和"融资融券类横切标签刷屏"两个痛点。

**入口**：`kg_analysis.dashboard_classification()`（分类快照，缓存键=族群日期+板块集，3s 轮询不重算）+ `dedup_dashboard_sectors()`（榜单去重），由 `realtime_engine.get_realtime_dashboard` 在 sector 模式调用（自选分组不去重）。

**三层规则与实测校准依据**：

| 规则 | 判定 | 校准实测（2026-08-18） |
|---|---|---|
| ① 枢纽过滤 | 成员 ≥300 只 且 族群浓度 max_share<0.55（成员散布多族群=横切标签） | 融资融券 0.30/深股通 0.31/沪股通 0.30/国企改革 0.35/专精特新 0.50 被标记；主题板块最低无人驾驶 0.61，两侧有余量。**n≥300 下限防小盘误判**（TOPCON电池 40 只浓度天然分散，初版曾被误隐藏） |
| ② 马甲折叠 | Jaccard≥0.2 连通组只留分最高者 | watched 内正好聚出 煤炭开采↔煤炭概念、HJT↔BC↔TOPCON、智能穿戴↔消费电子↔AI眼镜、CRO↔医疗研发外包 |
| ③ 族群限额 | 同 Louvain 族群每榜最多 2 席 | 半导体材料/设备/分立器件/MicroLED **成分零重叠**（Jaccard=0，产业链分工），只有族群维度能把它们折叠——这是引入本层的根本原因 |

**展示效果（2026-08-18 实盘 top10）**：半导体系从 4 席降到 2 席（材料+设备，MicroLED/分立器件折叠为"◈同类"徽标）；煤炭开采收编煤炭概念；折叠后从全量排序补位，榜单不缺行。前端：排行表 `◈同类N` 徽标 + 卡片区折叠行，悬停显完整清单；强弱榜同样适用。

**响应契约**：板块项新增 `similar: [{concept_code, concept_name, score}]` 与 `community_id`；顶层 `kg_dedup: {rules, top_folded, top_hidden_hubs, bottom_folded, bottom_hidden_hubs}`（null=无图谱未启用，此时榜单行为与旧版完全一致）。
