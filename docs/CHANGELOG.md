...
### 修复：实时看板刷新可见性与缓存新鲜度
- 板块强度和自选分组看板状态栏新增最近响应秒数，区分“页面每 3 秒请求”和“行情分钟粒度”。
- 新分时序列写入后立即失效同日期/模式的旧 `result_cache`，避免后台刷新成功后仍继续返回旧看板结果。
- 为 result cache 读写和失效增加共享锁及定向失效回归测试。

### 变更：成分股字段排序覆盖全部成员
- 新增 `GET /api/dashboard/members`：复用当前分时序列，对单个板块/自选分组全部有效成员按涨幅、涨速、加速、开盘至今或综合分排序，仅返回前 10。
- `MemberCardGrid` 点击列头时按需调用该接口；主看板 3 秒响应仍只携带 `members_top10`，避免响应体膨胀。
- 新行情分钟到达后自动更新已选择的全量排序；新增 12 支成员取全量排序前 10 的回归测试。

### 变更：监控板块成员数边界
- 管理候选、保存接口和实时引擎统一只接受最新成分股数 10~500（含边界）的板块。
- 实施时已用本机观察池和勾选集验证越界板块不参与运行、下次保存会被清除；具体勾选数量会随管理页保存变化，现值以 `/api/sector_manage/watched` 为准。
- 新增 9/10/500/501 四个边界的数据库回归测试。

### 重构：Dashboard 交互逻辑接入 composables
- `DashboardPage` 接入 `usePolling`、`useSession`、`usePlayTimeline`，删除页面内重复的轮询、会话和播放定时器。
- 统一轮询/手动刷新/历史计算的竞态序号；盘前暂停、10 秒恢复探测和收盘后继续轮询语义保持不变。
...
- 本机运行库经用户明确确认后已 DROP 退役 `watchlist` 表，删除 6,039 条 2026-06-12~2026-06-29 历史记录。
